...
export default function ConfirmationModal({ 
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div key="confirmation-modal-backdrop" className="fixed inset-0 z-[200] flex items-center justify-center p-4 overflow-hidden">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            {/* Background Accents */}
            <div className={`absolute top-0 right-0 w-48 h-48 rounded-full -mr-24 -mt-24 blur-3xl opacity-20 pointer-events-none ${
              type === 'danger' ? 'bg-red-600' : 
              type === 'warning' ? 'bg-amber-500' : 
              'bg-blue-500'
            }`}></div>
            <div className={`absolute bottom-0 left-0 w-48 h-48 rounded-full -ml-24 -mb-24 blur-3xl opacity-10 pointer-events-none ${
            ...
            <div className="flex flex-col items-center text-center relative z-10">
              <div className={`w-20 h-20 rounded-2xl flex items-center justify-center mb-6 shadow-lg relative ${
                type === 'danger' ? 'bg-red-600/10 text-red-600 shadow-red-600/20' : 
